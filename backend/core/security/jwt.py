...
class JWTHandler:
    ...
    def __init__(self):
        ...
        self.refresh_cookie_name = "refresh_token_cookie" # Новое имя куки
    ...
    async def set_refresh_token_cookie(self, response: Response, token: str) -> None:
        ...
        response.set_cookie(
            ...
            value=token,
            httponly=True,
            secure=self.settings.SESSION_COOKIE_SECURE if hasattr(self.settings, 'SESSION_COOKIE_SECURE') else True,
            samesite="lax",
...
async def get_current_user_payload(
    ...
    :return: Данные пользователя
    """
    jwt_handler = JWTHandler(settings) # Создаем экземпляр с настройками
...
async def get_current_active_user(
    ...
    :return: Пользователь
    """
    from core.models.user import User # Импорт здесь для избежания циклов
